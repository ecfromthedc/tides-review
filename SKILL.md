...
7. **Accessibility** — keyboard-only journey through every flow (a control unreachable by keyboard is a dead control wearing a pointer), focus order and focus traps on dialogs/routes, labels and roles for assistive tech, contrast that passes on the shipped palette not the design mock. A consumer surface unusable by keyboard or screen reader is a legal-grade defect class that renders invisible to every other lens here.
...
Report each finding as: `id · severity · file:line · one-line mechanism · concrete failure scenario · red-prove line · verdict (CONFIRMED / BANKED / DISMISSED)`, plus the angles that came back clean. **A red during a close is never dismissed as "flake" on vibes.** Any test that goes red mid-close and gets waved off must be proven flake: rerun it in isolation N times (or hunt the interleaving that makes it real) and name the reproduction or the mechanism in the report. An unproven flake-dismissal is itself a finding of the highest class — the two-clean-passes bar is only as strong as the honesty of the reds that got waved through it, and the reference project shipped a standing CVE-grade bug precisely because a red was called flake and dropped. **A single clean pass is not done** — re-run after fixing, and make the re-run a *hunt on the fixes themselves*, not a re-scan: fresh adversarial agents (different eyes, ideally a different model) tasked to break each fix, find second sites of the round's own new rules, and catch regressions the fixes introduced. On the reference close that pass found a MEDIUM in the round's own plumbing, two second-sites of its own rules, and the false red-proof — none visible to the lanes that produced them. The bar is **two consecutive passes that surface nothing new**. That is what turns "green" into an *earned* clean rather than a rubber stamp.
...
- **Concurrency & idempotency.** Hunt TOCTOU: every cap, uniqueness, or balance rule must be enforced *inside* the transaction that writes, with the row locked — a check outside the lock is a race with a polite error message. Every externally-triggered effect (webhook, payout, queue claim, delivery) must be provably once-only under replay AND under crash-between-two-writes: ask "if the process dies on this exact line, what does the retry do?" **Time is a shared resource in the same sense.** Clock skew between services, non-monotonic timestamps used for ordering, timezone/DST boundaries in "30 days"-style arithmetic, and overlapping schedulers/crons that assume they are alone break invariants that hold at every single instant — audit them as concurrency bugs, because at the boundary they are.
...
- **The bounds table.** Every outbound call has a timeout; every list endpoint has a limit; every ingest has a size ceiling; every retry has a cap and backoff; every queue claim has a lease. Grep the call sites and fill the table — an unbounded anything is a finding, because production will find it for you. **Then total the table into an aggregate cost envelope:** for anything that calls a paid provider, name the worst-case daily spend of the happy path *and* of the failure path (retry cap × fan-out × unit cost — five bounded retries across a 200-page fan-out is still an unbounded bill). An uncapped aggregate spend is a finding even when every individual bound is "correct," because the bill does not read your per-call timeouts.
...
4. **The go-live checklist**: exactly what stands between this state and production — credentials to set, env vars to fill, accounts to provision — each with where it plugs in. It also names the **soak window and trip-wires**: how long after ship someone watches (declare it — even 48 hours), which metric trips rollback vs. hotfix, and who is watching by name or role. A close that ends at "deployed green" is watching the first hour; most ship-killers arrive in the first two days, and "closed" means closed *and watched*, not signed at T0.
